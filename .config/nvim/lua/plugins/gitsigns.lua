...
--
-- Hunks only. Seeing the whole file's difference at once was <leader>gd here, built on
-- gitsigns.diffthis; that key is diffview's now and opens the same file in the view every other
-- <leader>g diff key opens -- see lua/plugins/diffview.lua. What is left is what gitsigns is for:
-- the sign column, and staging or discarding a hunk without leaving the buffer.
return {
  ...
  opts = {
    ...
    on_attach = function(bufnr)
      ...
      -- next/previous-difference behaviour -- including inside a diffview view, where the working
      -- file is an ordinary buffer this is still attached to and where losing ]c/[c would leave
      -- nothing to navigate the diff with.
      map("n", "]c", function()
        if vim.wo.diff then
