...
A search query typed in all lower case SHALL match regardless of case. A query containing an upper-case character SHALL match case-sensitively. Matches SHALL be shown and the view moved to them as the query is typed. Once a search is accepted, matches SHALL NOT remain highlighted.
...
#### Scenario: No lingering highlight

- **WHEN** the user accepts a search and returns to editing
- **THEN** no matches remain highlighted in the buffer
- **AND** no command is needed to clear a highlight
