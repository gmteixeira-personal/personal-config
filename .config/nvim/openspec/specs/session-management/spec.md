...
#### Scenario: The picker does not depend on what has been used first

- **WHEN** the user presses `<leader>qs` as the first mapping pressed in a session
- **THEN** the sessions are listed in the same fuzzy picker they would be listed in later in that session
- **AND** the listing is not degraded to a single-line prompt because the finder had not been loaded yet

#### Scenario: Searching with nothing saved

- **WHEN** the user presses `<leader>qs` and no session has been saved yet
- **THEN** the editor reports that there are none
- **AND** no picker is opened for the user to cancel out of
