...
Detail beyond orientation SHALL remain in `.config/nvim/openspec/specs/`, which the documentation SHALL name as the authoritative per-capability source, so the two cannot disagree about behaviour.
