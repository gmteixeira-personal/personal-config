...
The session's answer to "start a program without a terminal" is the launcher, and this program has no other route in: it declares no file type, so nothing opens it by association, and it appears in no menu it did not bring with it. An editor reachable only from a shell is reachable only by someone who did not need a window of their own.
