...
- [ ] 1.4 Exercise the three widgets in both vi modes — Ctrl+T inserts a path at the cursor without running the line, Ctrl+R replaces the line with a history entry without running it, Alt+C changes directory — and confirm dismissing each leaves the command line and working directory untouched.
