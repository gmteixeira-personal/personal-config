...
1. **Read the state.** Run these, and nothing else:
...
2. **Nothing to show?** If the dirty set is empty, print the status line alone — no legend, no tree — and stop. Do not add a sentence saying the tree is clean: `✅0 ✏️0 ❓0 💥0` already says it, and `⇡0 ⇣0` already says the branch is level.

3. **Work out session ownership.** Using the session-scope rule from the conventions, split the dirty paths into the ones this conversation created or modified through tool calls and the ones it did not.

   Unlike `/git:push`, an uncertain answer here is never a reason to ask. This command is read-only and must always print something. When the session edit record is unavailable or untrustworthy, leave the session column blank on every row and say once beneath the tree that ownership could not be determined — do not guess per file, and do not block on a question.

4. **Build the tree.** Split each path on `/` and merge the paths into one tree.
...
5. **Cap runaway untracked directories.** If a single untracked directory holds more than 20 files, do not list them. Show the directory with a count instead — `📁 build/ (147 untracked)`. The count in the tree is the whole report of it; do not add a sentence about it underneath.

6. **Render.** Print the legend, then the tree, then the status line — all inside one ```diff fenced block. See **Colour** below for why the fence is not optional.
